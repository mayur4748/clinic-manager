        ...
        <form id="appointmentEditForm" action="{{ route('appointments.update', $appointment->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="patient_name" value="{{ old('patient_name', $appointment->patient_name) }}" class="w-full border rounded-lg px-4 py-2 focus:ring focus:ring-blue-200" placeholder="Enter Patient Name">
                </div>
                <!-- CLINIC LOCATION -->
                <div>
                    <label class="block mb-2 font-semibold text-gray-700">
                    ...
                    <input type="text" name="clinic_location" value="{{ old('clinic_location', $appointment->clinic_location) }}" class="w-full border rounded-lg px-4 py-2 focus:ring focus:ring-blue-200" placeholder="Enter Clinic Location">
                </div>
                <!-- CLINICIAN -->
                <div>
                    <label class="block mb-2 font-semibold text-gray-700">
                    ...
                    <select name="clinician_id" class="w-full border rounded-lg px-4 py-2 focus:ring focus:ring-blue-200">
                        ...
                        @foreach($clinicians as $clinician)
                            <option value="{{ $clinician->id }}"
                                {{ old('clinician_id', $appointment->clinician_id) == $clinician->id ? 'selected' : '' }}>
                                {{ $clinician->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="appointment_date" value="{{ old('appointment_date', date('Y-m-d\TH:i', strtotime($appointment->appointment_date))) }}" class="w-full border rounded-lg px-4 py-2 focus:ring focus:ring-blue-200">
                </div>
                <!-- STATUS -->
                <div>
                    <label class="block mb-2 font-semibold text-gray-700">
